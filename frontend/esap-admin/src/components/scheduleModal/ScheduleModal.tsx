...
const ScheduleModal: React.FC<ScheduleModalProps> = ({ open, onClose, schedule }) => {
  ...
  return (
    ...
    >
      <Box sx={{margin: "auto", mt: 12, p: 3, width: "70%",
        height: "80%", bgcolor: "background.paper",
        ...
        <Grid container spacing={2} columns={14} textAlign="center">
          {daysOfWeek.map((day) => (
            <Grid key={day.toISOString()} item xs={2}>
              <ScheduleCard date={day.format("YYYY-MM-DD")} startTime={schedule.startDoctorAppointment}
                            endTime={schedule.endDoctorAppointment}  appointments={schedule.appointments}/>
            </Grid>
          ))}
        </Grid>
      </Box>
